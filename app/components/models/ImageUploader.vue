...
<template>
  <div class="space-y-4">
    ...
    >
      ...
      <UIcon name="i-heroicons-photo" :class="['size-12 mx-auto mb-4', isDragging ? 'text-primary' : 'text-muted']" />
      <p class="font-medium mb-1">Drop images here or click to browse</p>
      <p class="text-sm text-muted">JPG, PNG, WebP, or GIF. Max {{ formatBytes(maxImageSize) }} each.</p>
    </div>
    ...
    <div v-if="images.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      >
        ...
        <div class="aspect-square bg-elevated">
          <img :src="image.previewUrl" :alt="image.filename" class="w-full h-full object-cover" />
        </div>
        ...
        >
          ...
          <div class="flex gap-1">
            ...
            >
              <UButton size="xs" color="neutral" variant="solid" icon="i-heroicons-tag" @click.stop />
            </UDropdownMenu>
        ...
        <!-- Upload Status -->
        <div v-if="image.status === 'uploading'" class="absolute inset-0 bg-black/80 flex items-center justify-center">
          <div class="text-center">
            <UIcon name="i-heroicons-arrow-path" class="size-8 text-white animate-spin" />
        ...
        <div v-else-if="image.status === 'complete'" class="absolute top-2 left-2">
          <UIcon name="i-heroicons-check-circle-solid" class="size-5 text-success" />
        </div>
      </div>
    </div>

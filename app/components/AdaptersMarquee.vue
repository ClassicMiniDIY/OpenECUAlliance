  ...
  <div class="absolute isolate inset-0 overflow-hidden">
    <div class="flex flex-col justify-between pt-4">
      <div
        ...
        :class="rowIndex % 2 === 1 ? 'animate-marquee-reverse' : 'animate-marquee'"
      >
        <!-- First set of items -->
        <div
          ...
          :class="isLoaded ? 'opacity-100 scale-100 blur-0' : 'opacity-0 scale-50 blur-sm'"
        ...
        >
          ...
          />
          <UIcon v-else :name="getVendorIcon(adapter.vendor)" class="size-8 text-primary" />
        </div>
    ...
    <!-- Gradient overlays -->
    <!-- Left fade -->
    <div class="absolute left-0 top-0 bottom-0 w-1/3 z-10 bg-linear-to-r from-default to-transparent" />
    <!-- Right fade -->
    <div class="absolute right-0 top-0 bottom-0 w-1/3 z-10 bg-linear-to-l from-default to-transparent" />
    <!-- Bottom fade -->
    <div
...
<style scoped>
  ...
  .text-protection {
    background: radial-gradient(ellipse 70% 60% at 50% 55%, var(--ui-bg) 0%, var(--ui-bg) 15%, transparent 65%);
  }

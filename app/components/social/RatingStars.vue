...
<template>
  <div class="inline-flex items-center" :class="gapClasses">
    ...
    <div
      class="inline-flex"
      :class="[gapClasses, !readonly && 'cursor-pointer']"
    ...
    >
      ...
      >
        ...
        <UIcon
          ...
          class="absolute inset-0 text-muted/30"
          :class="sizeClasses"
        />

        <!-- Half star -->
        <div
          v-if="getStarFill(star) === 'half'"
    ...
    <span
      v-if="showValue"
      class="font-medium text-default ml-1"
      :class="textClasses"
    >
      {{ formattedValue }}
    </span>
    ...
    <!-- Count display -->
    <span v-if="showCount" class="text-muted ml-1" :class="textClasses">
      ({{ count }})
    </span>
  </div>
</template>

<script setup lang="ts">
  const { isAuthenticated, currentUser, profile, signOut, isAdmin, isModerator } = useAuth();

  const loading = ref(false);

  const avatarUrl = computed(() => profile.value?.avatar_url);
  const displayName = computed(() => profile.value?.display_name || profile.value?.username || 'User');
...
<template>
  ...
  <UButton v-if="!isAuthenticated" to="/login" color="primary" variant="soft" size="sm"> Sign in </UButton>
  ...
  >
    <UButton color="neutral" variant="ghost" class="p-1! flex items-center justify-center" :loading="loading">

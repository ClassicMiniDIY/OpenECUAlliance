<script setup lang="ts">
  ...
  const accessibleDescription = computed(() => {
    const desc = props.adapter.description || `Adapter for ${props.adapter.name} log files`;
  ...
  >
    <div class="flex flex-col h-full">
      ...
      <div class="flex items-start gap-3 mb-3">
        <div class="bg-muted/50 p-2.5 rounded-lg shrink-0 flex items-center justify-center size-10" aria-hidden="true">
          <img
          ...
          />
          <UIcon v-else :name="fallbackIcon" class="size-5 text-primary" aria-hidden="true" />
        </div>
      ...
      <div class="flex items-center justify-between text-sm text-muted pt-3 border-t border-default">
        <div class="flex items-center gap-1">
          <UIcon name="i-heroicons-queue-list" class="size-3.5" aria-hidden="true" />
          <span>{{ adapter.channelCount }} channels</span>
        </div>
        <UBadge color="neutral" variant="subtle" size="md"> v{{ adapter.version }} </UBadge>
      </div>
    </div>
  </UCard>
</template>

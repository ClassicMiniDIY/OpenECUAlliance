<script setup lang="ts">
      ...
      models.value = (data || []).map((model: any) => {
        ...
        return {
          ...
          primaryImageThumbnail: primaryImage?.thumbnail_path
            ? supabase.storage.from('model-images').getPublicUrl(primaryImage.thumbnail_path).data.publicUrl
            : null,
          recommendedMaterial: printing?.recommended_material || 'PLA',
          estimatedTimeHours: printing?.estimated_time_hours || null,
          fileFormats: [...new Set((model.model_files || []).map((f: any) => f.format))],
  ...
  async function deleteModel() {
    ...
    deleting.value = true;
    try {
      const { error } = await supabase.from('models').delete().eq('id', modelToDelete.value.id);
  ...
  <div>
    <UContainer class="py-12">
      ...
      <div v-else-if="filteredModels.length > 0" class="space-y-4">
        ...
        >
          <div class="flex gap-4">
            <!-- Image -->
            <div class="size-24 rounded-lg bg-elevated shrink-0 overflow-hidden">
              <img
              ...
              <div class="flex items-start justify-between gap-2">
                <div>
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    <UBadge :color="getStatusColor(model.status)" variant="subtle" size="xs">
                      {{ getStatusLabel(model.status) }}
                    </UBadge>
                    <UBadge v-if="model.featured" color="warning" variant="subtle" size="xs"> Featured </UBadge>
                  </div>
            ...
            <div class="flex flex-col gap-2 shrink-0">
              ...
              <UButton
                v-if="model.status === 'draft' || model.status === 'pending' || model.status === 'rejected'"
                variant="ghost"
                color="error"
                size="sm"
      ...
      <UCard v-else class="text-center py-12">
        <UIcon name="i-heroicons-cube" class="size-16 text-muted mx-auto mb-4" />
        <h3 class="font-semibold mb-2">
          {{ selectedStatus === 'all' ? 'No models yet' : `No ${selectedStatus} models` }}
        </h3>
        ...
        <UButton v-if="selectedStatus === 'all'" to="/models/upload"> Upload Your First Model </UButton>
      </UCard>
    </UContainer>
      ...
      <template #content>
        <UCard>
          ...
          <p class="text-muted">
            ...
            <span class="font-medium text-default">{{ modelToDelete?.name }}</span
            >? All files and images will be permanently removed.
          </p>

          <template #footer>
            <div class="flex justify-end gap-2">
              <UButton variant="outline" color="neutral" @click="deleteModalOpen = false"> Cancel </UButton>
              <UButton color="error" :loading="deleting" @click="deleteModel"> Delete Model </UButton>
            </div>
          </template>
        </UCard>
      </template>

...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.displayName !== (profile.value?.display_name ?? ""))
      data.displayName = form.displayName || undefined;
    if (form.bio !== (profile.value?.bio ?? ""))
      data.bio = form.bio || undefined;
    if (form.website !== (profile.value?.website ?? ""))
      data.website = form.website || undefined;
    if (form.location !== (profile.value?.location ?? ""))
        ...
        <UCard>
          <div class="space-y-6">
            ...
            <UFormField label="Username" required>
              <template #hint>
                <span v-if="usernameChecking" class="text-muted"
                  >Checking...</span
                >
                <span
                  v-else-if="
                  ...
                  >Available</span
                >
                <span v-else-if="usernameError" class="text-error">{{
                  usernameError
                }}</span>
              </template>
              <UInput
              ...
              <template #description>
                <span class="text-xs text-muted"
                  >Your unique username on OpenECU Alliance.</span
                >
              </template>
            </UFormField>
            ...
            <UFormField label="Display Name">
              ...
              <template #description>
                <span class="text-xs text-muted"
                  >This is how your name appears on your contributions.</span
                >
              </template>
            </UFormField>
            ...
            <UFormField label="Bio">
              ...
              <template #description>
                <span class="text-xs text-muted"
                  >{{ form.bio.length }}/500 characters</span
                >
              </template>
            </UFormField>
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              <UButton to="/profile" variant="ghost" :disabled="loading">
                Cancel
              </UButton>

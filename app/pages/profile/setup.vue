<script setup lang="ts">
  ...
  const canSubmit = computed(() => {
    return username.value.length >= 3 && usernameAvailable.value === true && !loading.value && !checking.value;
  });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      submitError.value = err instanceof Error ? err.message : 'Failed to create profile';
    } finally {
      loading.value = false;
    }
  }
  ...
  <div class="min-h-[calc(100vh-4rem)] flex items-center justify-center py-12 px-4">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-muted mt-2">Choose a username to get started. This will be visible to others.</p>
      </div>
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <UFormField label="Username" required class="mb-4">
            <template #hint>
              ...
              <span v-else-if="usernameAvailable === true" class="text-success">Available</span>
            ...
            <UInput
              ...
              size="lg"
              :color="error ? 'error' : usernameAvailable === true ? 'success' : 'neutral'"
              required
              :disabled="loading"
            >
              <template #leading>
              ...
              <template #trailing>
                <UIcon v-if="checking" name="i-heroicons-arrow-path" class="size-4 animate-spin text-muted" />
                <UIcon
            ...
            <template #description>
              <span class="text-xs text-muted">3+ characters. Letters, numbers, underscores, and hyphens only.</span>
            </template>
          </UFormField>

          <!-- Display Name -->
          <UFormField label="Display name" class="mb-6">
            <UInput v-model="displayName" placeholder="Your Name (optional)" size="lg" :disabled="loading" />
            <template #description>
              <span class="text-xs text-muted">This is how your name will appear on your contributions.</span>
            </template>
          </UFormField>

          <UButton type="submit" block size="lg" :loading="loading" :disabled="!canSubmit"> Create profile </UButton>
        </form>
      </UCard>
    </div>
  </div>

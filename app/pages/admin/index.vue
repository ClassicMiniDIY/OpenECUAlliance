<script setup lang="ts">
  ...
  useSeoMeta({
    ...
    description: 'OpenECU Alliance admin dashboard for managing content, users, and moderation queue.',
  ...
  async function loadData() {
    const [statsData, queueData] = await Promise.all([fetchStats(), fetchQueue({ status: 'pending', limit: 5 })]);
  ...
  <div>
    <UContainer class="py-12">
      ...
      <div class="mb-8">
        ...
        <p class="text-muted">Overview of moderation activity and site statistics</p>
      </div>
      ...
      <template v-else>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          >
            <div class="flex items-center gap-4">
              <div :class="`size-12 rounded-xl bg-${stat.color}/10 flex items-center justify-center`">
        ...
        <!-- Quick Actions -->
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <UButton to="/admin/queue" block size="lg" variant="outline" icon="i-heroicons-queue-list">
            Moderation Queue
          </UButton>
          <UButton to="/admin/reports" block size="lg" variant="outline" icon="i-heroicons-flag"> Reports </UButton>
          <UButton to="/admin/users" block size="lg" variant="outline" icon="i-heroicons-users">
            User Management
          </UButton>
          <UButton to="/models" block size="lg" variant="outline" icon="i-heroicons-cube"> Browse Models </UButton>
        </div>
          ...
          <UCard>
            <template #header>
              <div class="flex items-center justify-between">
                ...
                <UButton to="/admin/queue" variant="ghost" size="sm"> View All </UButton>
              </div>
            </template>

            <div v-if="recentQueue.length > 0" class="space-y-4">
              <div v-for="item in recentQueue" :key="item.id" class="flex items-center gap-4">
                <!-- Image -->
                <div class="size-12 rounded-lg bg-elevated shrink-0 overflow-hidden">
                  <img
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex items-center gap-2 text-sm text-muted">
                    <span>by {{ item.model.author.displayName || item.model.author.username }}</span>
                ...
                <!-- Action -->
                <UButton :to="`/admin/queue/${item.id}`" variant="ghost" size="sm" icon="i-heroicons-arrow-right" />
              </div>
            </div>

            <div v-else class="text-center py-8 text-muted">
              <UIcon name="i-heroicons-check-circle" class="size-8 mx-auto mb-2" />

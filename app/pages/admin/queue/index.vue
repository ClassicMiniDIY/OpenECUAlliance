...
const statusFilter = ref<"all" | "pending" | "in_progress" | "resolved">(
  (route.query.status as any) || "pending",
);
const typeFilter = ref<
  "all" | "new_submission" | "update" | "report" | "appeal"
>("all");
...
const filteredQueue = computed(() => {
  return queue.value.filter((item) => {
    if (statusFilter.value !== "all" && item.status !== statusFilter.value)
      return false;
    if (typeFilter.value !== "all" && item.actionType !== typeFilter.value)
      return false;
    if (
      assignedFilter.value === "mine" &&
      item.assignedTo !== currentUser.value?.id
    )
      return false;
    if (assignedFilter.value === "unassigned" && item.assignedTo !== null)
      return false;
    return true;
  });
});
    ...
    <UContainer class="py-12">
      ...
      <div v-else-if="filteredQueue.length > 0" class="space-y-4">
        ...
        >
          <div class="flex gap-4">
            <!-- Image -->
            <div
              class="size-20 rounded-lg bg-elevated shrink-0 overflow-hidden"
            >
              <img
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex flex-wrap items-center gap-4 text-sm text-muted">
                <div class="flex items-center gap-1">
                  ...
                  />
                  <span>{{
                    item.model.author.displayName || item.model.author.username
                  }}</span>
                </div>
                <div class="flex items-center gap-1">
      ...
      <UCard v-else class="text-center py-12">
        <UIcon
          ...
          class="size-16 text-success mx-auto mb-4"
        />

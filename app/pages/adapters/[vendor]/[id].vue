...
useSeoMeta({
  title: () => adapter.value ? `${adapter.value.name} - OpenECU Alliance` : 'Adapter - OpenECU Alliance',
    ...
    <UContainer class="py-8">
      ...
      <div v-else>
        ...
        <div class="flex flex-col sm:flex-row items-start gap-4 mb-8">
          <div class="bg-muted/50 p-4 rounded-xl shrink-0">
        ...
        <UCard class="mb-8">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <div class="font-medium uppercase">{{ adapter.fileFormat.type }}</div>
            </div>
            <div>
              <div class="text-muted mb-1">Extensions</div>
              <div class="font-medium">{{ adapter.fileFormat.extensions.join(', ') }}</div>
            </div>
            <div v-if="adapter.fileFormat.delimiter">
              <div class="text-muted mb-1">Delimiter</div>
              <div class="font-medium font-mono">
                {{ adapter.fileFormat.delimiter === ',' ? 'comma' : adapter.fileFormat.delimiter === ';' ? 'semicolon' : adapter.fileFormat.delimiter }}
              </div>
            </div>
            <div v-if="adapter.fileFormat.endianness">
              <div class="text-muted mb-1">Endianness</div>
              <div class="font-medium capitalize">{{ adapter.fileFormat.endianness }}</div>
            </div>
          </div>
        </UCard>

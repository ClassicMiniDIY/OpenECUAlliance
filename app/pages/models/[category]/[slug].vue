<script setup lang="ts">
  ...
  const { data: model, status } = await useFetch<UserModelDetail>(`/api/models/${category}/${slug}`, {
  ...
  const fallbackIcon = computed(() =>
    model.value?.vendor ? getVendorIcon(model.value.vendor) : getCategoryIcon(category as any)
  );

  // Primary image
  const primaryImage = computed(() => {
    if (!model.value?.images?.length) return null;
    return model.value.images.find((img) => img.isPrimary) || model.value.images[0];
  });
  ...
  const formattedPrintTime = computed(() => {
    ...
    const minutes = Math.round((model.value.printing.estimatedTimeHours - hours) * 60);
  ...
  <div>
    <UContainer class="py-8">
      ...
      <nav class="mb-6" aria-label="Breadcrumb">
        <ol class="flex items-center gap-2 text-sm text-muted">
          <li v-if="isUnpublished">
            <NuxtLink to="/profile/models" class="hover:text-primary"> My Models </NuxtLink>
          </li>
          <template v-else>
            <li>
            ...
            <li>
              <NuxtLink :to="`/models?category=${category}`" class="hover:text-primary capitalize">
      ...
      <div v-if="status === 'pending'" class="flex items-center justify-center py-24">
        <UIcon name="i-heroicons-arrow-path" class="size-8 text-warning animate-spin" />
      </div>
      ...
      <UCard v-else-if="status === 'error'" class="text-center py-12">
        ...
        <p class="text-muted mb-4">The model "{{ slug }}" could not be found.</p>
      ...
      >
        ...
        <div>
          ...
          <p class="text-sm text-muted">
            This model is awaiting moderator approval. Social features are disabled until it's published.
          </p>
        </div>
      </div>
      ...
      <div v-if="model" class="grid lg:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div class="aspect-video bg-muted/30 rounded-xl overflow-hidden mb-4">
            <img v-if="primaryImage" :src="primaryImage.url" :alt="model.name" class="w-full h-full object-cover" />
            <div v-else class="w-full h-full flex items-center justify-center">
              <UIcon :name="getCategoryIcon(model.category)" class="size-24 text-muted" />
            </div>
          </div>

          <!-- Image Gallery -->
          <div v-if="model.images && model.images.length > 1" class="grid grid-cols-4 gap-2 mb-6">
            <div
            ...
            >
              <img :src="img.thumbnailUrl || img.url" :alt="model.name" class="w-full h-full object-cover" />
            </div>
          </div>
          ...
          <UCard v-if="model.isLinked && model.externalUrl">
            ...
            <div class="space-y-4">
              <p class="text-sm text-muted">
                This model is hosted externally. Click below to view and download on the original platform.
              </p>
              <UButton
                ...
                block
              >
                <LinkedModelBadge v-if="model.externalPlatform" :platform="model.externalPlatform" class="mr-2" />
                View on
                {{ model.externalPlatform ? EXTERNAL_PLATFORMS[model.externalPlatform]?.name : 'Original Site' }}
                <UIcon name="i-heroicons-arrow-top-right-on-square" class="size-4 ml-2" />
              </UButton>
              <div v-if="model.externalAuthorName" class="text-xs text-muted text-center">
          ...
          <UCard v-else>
            <template #header>
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs text-muted"> {{ model.downloadsCount }} downloads </span>
              </div>
            </template>
            <div class="space-y-2">
              <a
              ...
              >
                ...
                <UIcon name="i-heroicons-arrow-down-tray" class="size-5 text-warning" />
              </a>
            </div>
          </UCard>
        </div>
        ...
        <div>
          ...
          <div class="flex items-start gap-4 mb-6">
            <div class="bg-warning/10 p-3 rounded-xl shrink-0" aria-hidden="true">
              ...
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    <UBadge v-if="isPending" color="warning" variant="outline" class="shrink-0">
                      Pending Review
                    </UBadge>
                    <UBadge v-if="model.featured" color="primary" variant="subtle" class="shrink-0"> Featured </UBadge>
                    <LinkedModelBadge
                  ...
                  <div class="flex items-center gap-2 text-sm text-muted">
                    <span v-if="model.vendor" class="capitalize">{{ model.vendor }}</span>
                ...
                <div class="shrink-0">
                  ...
                  <UTooltip v-else text="Available after approval">
                    <UButton color="neutral" variant="ghost" disabled class="opacity-50">
          ...
          <div class="flex items-center gap-4 mb-6 text-sm text-muted">
            ...
            <span v-if="model.averageRating > 0" class="flex items-center gap-1">
              <UIcon name="i-heroicons-star-solid" class="size-4 text-warning" />
          ...
          <UCard class="mb-6">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
              ...
              <div v-if="model.printing.layerHeight">
                ...
                <div class="font-medium">{{ model.printing.layerHeight }}mm</div>
              </div>
          ...
          <UCard v-if="model.hardware?.length" class="mb-6">
            ...
            <ul class="space-y-2">
              <li v-for="hw in model.hardware" :key="hw.item" class="flex items-start gap-2 text-sm">
          ...
          <UCard v-if="model.assembly?.steps?.length">
            <template #header>
              <div class="flex items-center justify-between">
                ...
                <UBadge v-if="model.assembly.difficulty" color="neutral" variant="subtle">
          ...
          <div class="mt-6 text-xs text-muted flex items-center gap-4">
            ...
            <span v-if="model.commercialUseAllowed">Commercial use allowed</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="model" class="mt-12 grid lg:grid-cols-2 gap-8">
        ...
        <UCard :class="{ 'opacity-60': isUnpublished }">
          <template #header>
            <div class="flex items-center justify-between">
              ...
              <UBadge v-if="isUnpublished" color="neutral" variant="outline" size="xs"> Disabled </UBadge>
            </div>
          </template>
          <div v-if="isUnpublished" class="text-center py-6 text-muted">
            <UIcon name="i-heroicons-star" class="size-8 mx-auto mb-2 opacity-50" />
        ...
        <UCard :class="{ 'opacity-60': isUnpublished }">
          ...
          <div v-if="isUnpublished" class="text-center py-6 text-muted">
            <UIcon name="i-heroicons-chat-bubble-left" class="size-8 mx-auto mb-2 opacity-50" />

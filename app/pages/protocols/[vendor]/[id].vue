...
const { data: protocol, status } = await useFetch<ProtocolDetail>(
  `/api/protocols/${vendor}/${id}`,
  {
...
const formattedBaudrate = computed(() =>
  protocol.value ? formatBaudrate(protocol.value.protocol.baudrate) : "",
);

const totalSignals = computed(() => {
  if (!protocol.value) return 0;
  return protocol.value.messages.reduce(
    (total, msg) => total + msg.signals.length,
    0,
  );
});
    ...
    <UContainer class="py-8">
      ...
      <nav class="mb-6" aria-label="Breadcrumb">
        <ol class="flex items-center gap-2 text-sm text-muted">
          <li>
            <NuxtLink to="/protocols" class="hover:text-primary">
              Protocols
            </NuxtLink>
          </li>
      ...
      >
        <UIcon
          ...
          class="size-8 text-success animate-spin"
        />
      </div>
      ...
      <div v-else-if="protocol">
        ...
        <div class="mb-8">
          ...
          <div class="space-y-3">
            <UCard
              v-for="msg in protocol.messages"
              :key="String(msg.id)"
            ...
            >
              ...
              >
                <div class="flex items-center gap-4">
                  <code
                    ...
                    @click.stop="
                      copyToClipboard(formatMessageId(msg.id), String(msg.id))
                    "
                  >
                    {{ formatMessageId(msg.id) }}
                    <UIcon
                      v-if="copied === String(msg.id)"
                  ...
                  <div>
                    ...
                    <div class="text-sm text-muted">
                      {{ msg.length }} bytes
                      <span v-if="msg.intervalMs">
                        • {{ msg.intervalMs }}ms interval
                      </span>
                ...
                <UIcon
                  :name="
                    expandedMessages.has(String(msg.id))
              ...
              <!-- Signals (expanded) -->
              <div
                v-if="expandedMessages.has(String(msg.id))"
        ...
        <div v-if="protocol.metadata" class="grid md:grid-cols-2 gap-6">
          ...
          <UCard v-if="protocol.metadata.testedWith?.length">
            ...
            <ul class="space-y-1">
              <li
                v-for="ecu in protocol.metadata.testedWith"
                :key="ecu"
          ...
          <UCard v-if="protocol.metadata.compatibleTools?.length">
            ...
            <div class="flex flex-wrap gap-2">
              <UBadge
                v-for="tool in protocol.metadata.compatibleTools"
                :key="tool"
                color="neutral"
                variant="subtle"
              >

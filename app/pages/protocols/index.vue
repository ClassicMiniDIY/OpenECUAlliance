<script setup lang="ts">
  ...
  const { protocols, vendors, protocolTypes, filterProtocols, loading, formatBaudrate } = useProtocols();
  ...
  <div>
    <UContainer class="py-12">
      ...
      <div class="text-center max-w-3xl mx-auto mb-12">
        <UBadge color="success" variant="subtle" class="mb-4"> CAN Bus Protocols </UBadge>

        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-4">Protocol Definitions</h1>

        <p class="text-lg text-muted mb-8">
          Browse CAN Bus protocol definitions for real-time ECU communication. Download as DBC for use with industry
          tools.
        </p>
      ...
      <div class="flex justify-between items-center mb-4 text-sm text-muted">
        ...
        <NuxtLink to="/contribute" class="hidden md:flex items-center gap-1 hover:text-success transition-colors">
          Submit a protocol
      ...
      <div v-if="loading" class="flex flex-col items-center justify-center py-24" role="status" aria-busy="true">
        <UIcon name="i-heroicons-arrow-path" class="size-8 text-success animate-spin mb-4" />
        <p class="text-muted">Loading protocols...</p>
      </div>

      <!-- Protocol Grid -->
      <div v-else-if="filteredProtocols.length > 0" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <ProtocolCard v-for="protocol in filteredProtocols" :key="protocol.id" :protocol="protocol" />
      </div>

      <!-- Empty State -->
      <UCard v-else class="text-center py-12">
        <UIcon name="i-heroicons-signal-slash" class="size-12 text-muted mx-auto mb-4" />
        <h3 class="font-semibold mb-2">No protocols found</h3>
        <p class="text-muted mb-4">There's no protocol matching your search yet. Be the first to create it!</p>
        <div class="flex gap-2 justify-center">
          <UButton color="success" variant="soft" to="/contribute"> Contribute on GitHub </UButton>
